...
INSERT INTO address (`add`, nameReceiver, phoneNumber)
VALUES ('Hà nội', 'Nguyễn văn A', '0987736533');

-- Thêm dữ liệu vào bảng User
INSERT INTO `user` (fullname, username, password, address_id)
VALUES ('Xuân Hùng', 'hunghx', '123456', 1);

-- Thêm dữ liệu vào bảng Order
INSERT INTO `order` (user_id, total)
VALUES (1, 1000000), -- Đơn hàng 1 của người dùng có id = 1, tổng tiền = 1.000.000
       (2, 750000),  -- Đơn hàng 2 của người dùng có id = 2, tổng tiền = 750.000
       (3, 500000),  -- Đơn hàng 3 của người dùng có id = 3, tổng tiền = 500.000
       (4, 900000);
-- Đơn hàng 4 của người dùng có id = 4, tổng tiền = 900.000

-- Thêm dữ liệu vào bảng Product
INSERT INTO product (name, price, quantity, category_id)
VALUES ('Quần âu nam', 150000, 10, 1),
...
INSERT INTO order_detail (order_id, product_id, productName, productPrice, quantity)
VALUES (1, 1, 'Quần âu nam', 150000, 2),
       (1, 3, 'Áo dài', 550000, 5),
...
SELECT `Order`.id,
       `Order`.createdDate        AS ngayTao,
       `User`.fullname            AS hoTen,
       SUM(Order_detail.quantity) AS soLuong,
       `Order`.total              AS tongTien
FROM `Order`
         LEFT JOIN `User` ON `Order`.user_id = `User`.id
...
WHERE Category.name = 'Áo'
  AND Product.quantity > 5;
